    ...
    return (
      <main className="relative z-[1] mx-auto max-w-[1400px] px-6 py-8">
        ...
        <div className="flex gap-4 overflow-hidden">
          <Skeleton className="h-[320px] w-[290px] shrink-0 rounded-[14px]" />
          <Skeleton className="h-[320px] w-[290px] shrink-0 rounded-[14px]" />
          <Skeleton className="h-[320px] w-[290px] shrink-0 rounded-[14px]" />
        </div>
      </main>
...
function ScrollCard({ comp }: { comp: CompetitionSummary }) {
  ...
  return (
    <Link
      href={`/competitions/${comp.id}`}
      className="group flex w-[290px] flex-shrink-0 flex-col overflow-hidden rounded-[14px] border border-[var(--border-default)] bg-[var(--bg-card)] backdrop-blur-md transition hover:-translate-y-[3px] hover:border-[var(--border-hover)] hover:shadow-[0_12px_32px_var(--accent-glow)]"
      style={{ scrollSnapAlign: "start" }}
    >
      <div className="relative h-[125px] shrink-0 overflow-hidden bg-gradient-to-br from-[var(--bg-surface-dim)] to-[var(--bg-surface)]">
        {hasBanner ? (
          <img
            src={assetUrl((comp.bannerUrl || comp.mobileBannerUrl)!)}
            alt=""
      ...
      <div className="flex flex-1 flex-col p-3.5 pt-3">
        <h3 className="mb-1 line-clamp-2 text-[15px] font-bold leading-snug text-[var(--text-primary)] group-hover:text-accent-primary">
          {comp.title}
        </h3>
        {comp.description && (
          <p className="mb-2.5 line-clamp-2 text-xs leading-relaxed text-[var(--text-tertiary)]">
            {comp.description}
          </p>
        )}
        {comp.eventTypes && comp.eventTypes.length > 0 && (
        ...
        <div className="mt-auto flex items-center justify-between border-t border-[var(--border-dim)] pt-2.5">
          {dateLabel && (
